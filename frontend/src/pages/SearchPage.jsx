...
function SearchPage() {
    ...
    const handleSearch = async () => {
        ...
        try {
            const response = await fetch(`http://127.0.0.1:8000/skill?skill=${skill}`);
            if (!response.ok) {
                throw new Error(`Ошибка HTTP: ${response.status}`);
            }
            const data = await response.json();
            setVacancies(data.vacancies || []); // Assuming response format contains "vacancies" field
        } catch (error) {
        ...
        <Center mt={"30px"}>
            <Flex flexDir={"column"} gap={'20px'}>
                <Box display={"flex"} gap={"20px"} justifyContent="center">
                        <Input w="500px" placeholder='Поиск по скилам' value={skill} onChange={handleInputChange} />
                </Box>
                <Center>
